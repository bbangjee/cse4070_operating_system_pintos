...
thread_set_priority (int new_priority) 
{
  ...
  if (!list_empty(&ready_list)) {
    ...
    if (new_priority < highest_priority_thread->priority) thread_yield();
  }
}
...
{
  int temp = FP_MUL_INT(load_avg, 100);
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  for (int i = 0; i < 128; i++) t->fdt[i] = NULL;
#endif
}
...
void thread_wakeup(void) {
  ...
  if (next_wakeup_tick > current_tick) return;
  ...
  for (struct list_elem *e = list_begin(&sleep_thread_list); e != list_end(&sleep_thread_list);) {
    ...
    if (t->wakeup_time <= current_tick) {
      ...
      if (new_min > t->wakeup_time) new_min = t->wakeup_time;
      e = list_next(e);
    }
    
  }
  next_wakeup_tick = new_min;
}
...
void thread_preemption(struct thread* cur) {
  if (list_empty(&ready_list)) return;
  struct thread* thread_to_check =
      list_entry(list_front(&ready_list), struct thread, elem);
  if (cur->priority < thread_to_check->priority) {
    thread_yield();
  }
}
...
void calculate_priority(struct thread* t) {
  ...
  new_priority = FP_SUB_FP(new_priority, recent_cpu_div_4);
  new_priority = FP_SUB_INT(new_priority, nice_mul_2);

  int new_priority_int = FP_TO_INT_NEAREST(new_priority);

  if (new_priority_int > PRI_MAX) new_priority = PRI_MAX;
  if (new_priority_int < PRI_MIN) new_priority = PRI_MIN;
  t->priority = new_priority_int;
}

void calculate_recent_cpu(struct thread* t) {
  if (t == idle_thread) return;
  // 공식: recent_cpu = (2*load_avg)/(2*load_avg + 1) * recent_cpu + nice
  int mul2_load_avg = FP_MUL_INT(load_avg, 2);
  int mul2_load_avg_plus1 = FP_ADD_INT(mul2_load_avg, 1);
  int temp = FP_DIV_FP(mul2_load_avg, mul2_load_avg_plus1);
  temp = FP_MUL_FP(temp, t->recent_cpu);
...
void calculate_load_avg(void) {
  ...
  int temp01 = FP_DIV_INT(INT_TO_FP(59), 60);
  int temp02 = FP_DIV_INT(INT_TO_FP(1), 60);

  load_avg = FP_ADD_FP(FP_MUL_FP(temp01, load_avg), FP_MUL_INT(temp02, ready_threads));
}
...
void recalculate_all_priority(void) {
  ...
  struct thread* highest_priority_thread =
      list_entry(list_front(&ready_list), struct thread, elem);
  int highest_priority = highest_priority_thread->priority;
  struct thread* cur = thread_current();
