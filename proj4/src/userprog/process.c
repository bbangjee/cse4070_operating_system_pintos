...
tid_t process_execute(const char* file_name) {
  ...
  char fn_tmp[128];
  strlcpy(fn_tmp, file_name, sizeof(fn_tmp));
  cmd_name = strtok_r(fn_tmp, " ", &temp_ptr);
  if (cmd_name == NULL) {
    free(fn_copy);
    return TID_ERROR;
  }
...
static bool load_segment(struct file* file, off_t ofs, uint8_t* upage,
  ...
  while (read_bytes > 0 || zero_bytes > 0) {
    ...
    vme->mapped_file = file_reopen(file);
    if (vme->mapped_file == NULL) {
      free(vme);
      return false;
    }
...
bool handle_mm_fault(struct vm_entry* vme) {
  // 페이지 폴트 발생시 물리페이지를 할당 Disk에 있는 file을 물리페이지로 load
