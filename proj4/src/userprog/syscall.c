...
static void syscall_handler(struct intr_frame* f) {
  ...
  switch (syscall_number) {
    case SYS_HALT:
      ...
      break;
    case SYS_EXIT:
      check_address(esp + 1, f->esp);
      sys_exit((int)(*(esp + 1)));
      break;
    case SYS_EXEC:
      check_address(esp + 1, f->esp);
      check_valid_string((const char*)(*(esp + 1)), f->esp);
    ...
    case SYS_READ:
      check_address(esp + 1, f->esp);
      check_address(esp + 2, f->esp);
      check_address(esp + 3, f->esp);
      check_valid_buffer((void*)(*(esp + 2)), (unsigned)(*(esp + 3)), f->esp,
                         true);
      f->eax = sys_read((int)(*(esp + 1)), (const void*)(*(esp + 2)),
                        (unsigned)(*(esp + 3)));
      break;
    case SYS_WRITE:
      check_address(esp + 1, f->esp);
    ...
    case SYS_MAX_OF_FOUR_INT:
      ...
      check_address(esp + 2, f->esp);
      check_address(esp + 3, f->esp);
      check_address(esp + 4, f->esp);
      f->eax = max_of_four_int((int)(*(esp + 1)), (int)(*(esp + 2)),
                               (int)(*(esp + 3)), (int)(*(esp + 4)));
      break;
    case SYS_CREATE:
      check_address(esp + 1, f->esp);
...
int sys_exec(const char* cmd_line) { return process_execute(cmd_line); }
...
struct vm_entry* check_address(void* vaddr, void* esp) {
  ...
  struct vm_entry* vme = find_vm_entry(vaddr);
...
void check_valid_buffer(void* buffer, unsigned size, void* esp, bool to_write) {
  ...
  char* buf_end = buf_ptr + size;

  while (buf_ptr < buf_end) {
    struct vm_entry* vme = check_address(buf_ptr, esp);
    if (to_write && (vme == NULL || !vme->writable)) {
      sys_exit(-1);
    }
    buf_ptr = (char*)pg_round_down(buf_ptr) + PGSIZE;
  }

  if (size > 0) {
    check_address(buf_end - 1, esp);
  }
}
...
void check_valid_string(const char* str, void* esp) {
  ...
  check_address(ptr, esp);  // 널 종료 문자 검사
}